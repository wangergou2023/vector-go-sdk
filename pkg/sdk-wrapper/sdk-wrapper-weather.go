...
func DisplayTemperature(temp int, unit string, delay int, blocking bool) error {
	digit01 := int(temp / 10)
	digit02 := int(temp % 10)
	digit01File, err1 := os.Open("weather/weather_temp_" + fmt.Sprintf("%d", digit01) + ".png")
	digit02File, err2 := os.Open("weather/weather_temp_" + fmt.Sprintf("%d", digit02) + ".png")
	unitFile, err3 := os.Open("weather/weather_celsius_indicator.png")
	if unit == "f" {
		unitFile, err3 = os.Open("weather/weather_fahrenheit_indicator.png")
	}
	signFile, err4 := os.Open("weather/weather_negative_indicator.png")
